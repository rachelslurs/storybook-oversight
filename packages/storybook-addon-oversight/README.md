...
- **Storybook ^10.3** (React projects), and exactly one resolved copy of `@storybook/addon-docs` within that range. Two copies carry two theme contexts, so the Docs page renders blank with errors that name neither this addon nor the duplicate ([#93](https://github.com/rachelslurs/storybook-oversight/issues/93)). Mixing versions across the Storybook packages is the usual way to end up with two; from `0.6.1` the addon warns at startup when it detects it.
