...
- 4ef0f14: Read a component's description from the manifest entry only, never from the docgen payload.
...
- 00531df: Read the docgen payload Storybook's MCP serves, when an entry carries more than one.
...
- 197d9ed: Narrow the Storybook peer range to `^10.5.0`. It claimed `^10.3.0`, and on 10.3 and 10.4 the Docs block throws while rendering and takes the whole Docs page down with it, so the page shows nothing at all rather than showing without the block. Nothing about that failure points at this addon.
...
- be41faa: The panel and the Docs block now read the manifest wherever it lives. A v:1 ref-based manifest (what `experimentalDocgenServer` builds write) resolves its per-component refs over fetch, and in dev under that flag, where the manifest route 404s on purpose, both surfaces read the component index from Storybook's in-runtime service API instead. Projects without the flag keep the exact fetch path they had.

## 0.4.1

### Patch Changes

- 8dd1c89: Ship type declarations for the `manager` and `preset` entry points. Both resolved to JavaScript with no types, so anything reading the package's type surface saw two of its three entries as untyped. `blocks`, the entry consumers actually import, was already typed and is unchanged.
...
- One word per thing. The two surfaces are the addons panel and the Docs block, which the addon called the manager panel and the Docs-page block in places. A rule dictates a finding, whose parts are a severity, a rule name, a message and a hint. Finding is the only word for it now: the type was `Diagnostic`, the CLI tally said "problems", and prose said "issues". `oversight-core` is the rules engine.
  ...
  Two strings consumers might match on changed. `component-description-missing` reads `<name> has no description for the MCP or the Docs page to show.`, and the CLI's tally line reads `✖ 5 findings (2 errors, 2 warnings, 1 info)` where it read `5 problems`.

  The addon README heading is now `Optional: enable the Docs block`, so `#optional-enable-the-docs-page-block` no longer resolves.

### Patch Changes

- A report opens with the component's description, then its findings, then its props. The findings used to come first, which asked the reader to judge what was wrong with a component before seeing what it says it is. Headings and the space between them carry the division, where a rule between each section drew three boxes inside the box the panel and the Docs block already draw.

- Findings read as a table: rule, severity, message, and a hint the last column reveals from a lightbulb, on pointer or on keyboard focus. Every finding carries the one-line hint for its rule on a new `hint` field, distilled from `docs/troubleshooting.md`. It lives with the rules rather than in a renderer, so both surfaces give the same answer, and a rule added to the union has to say what to do about itself or fail to compile. `deprecated-tag` has none: it reports a fact rather than a defect. The lightbulb names itself with the hint text, so the fix is read out whether or not it is opened.

  The field is `hint` rather than `fix` because ESLint's `fix` is a machine-applicable edit that `--fix` applies, and this is a sentence to read.

- The Props section lists every prop in a table, in the treatment the Controls panel gives its own: a muted heading over each column, a rule between rows, and no cell borders or striping on either surface. Each row says whether the prop is required, in a word, and whether it is documented, as a tick or a cross carrying its own label rather than a glyph and a color alone.

  Neither table takes a heading over it, since its columns name it, and the props table's first column is headed `Prop`, which said the same thing twice. The heading stays on the two cases with no table to say it: a component with no props extracted, and a manifest whose prop payload was not recognized.

- A finding names the props it is about. The report gave a count while the props table below gave the names, so reading one meant crossing it against the other.

- Every row says what it is about. The prop name and the rule name were plain cells, so a screen reader reading the Documented column announced the column and the verdict and never which prop. Both are row headings now, and the tick and the cross name the prop with them: `children is documented` where they read `documented`, which only repeated the column heading announced right before them.

  Each section label is a heading too. `Description`, `Manifest`, `Props` and `Extraction` were bold text and nothing more, so heading navigation skipped the whole report on both surfaces.

  Everything a report says with a glyph it says in words. A dangling `?path=` link in a description was struck through and marked with a warning sign carrying no label, so the mark meant nothing to a screen reader. The tab's count badge read as "Oversight 2"; it reads as "Oversight 2 findings".

- Each table is a named region with a tab stop and scrolls inside its own box rather than spilling out of the section on a narrow panel. Nothing inside them took focus, so a table with more columns than room was one a keyboard could neither reach nor scroll.

- One color per role, on both themes. Section headings, warning and error text and struck manifest ids set no color of their own, and the section painting the background behind them set none either, so all of it fell back to the browser's black: legible on a light page and all but invisible in the Docs block on a dark one. The text tones that did apply hold one value for both themes, so a struck id in a `docs-link-dangling` finding sat at 1.64:1 on a dark Docs page and the mark beside it at 2.83:1. Both take the semantic foreground scale now, the one Storybook's own badges use.

  Report text also takes the size of what it sits in on a Docs page, which sizes every span and div it does not recognize at 16px.

- The Docs block opens with a section heading rather than a caption bar inside its own border, so it reads as part of the page. It is the Docs page's own heading component, so it carries the same copy-the-URL control every other heading there has, and it holds `id="oversight"` for a fixed `#oversight` anchor on any component's Docs page. Only one block per page claims it.

  The block renders a component's description the way the panel does, as the prose itself. It showed a "Documented" verdict instead, on the reasoning that the Docs page prints the description higher up, which left a `docs-link-dangling` finding's strikethrough nowhere to land.

  A `?path=` redirect in that description is a link there rather than plain text. The block passed no link component and the renderer falls back to a bare label without one, so the same redirect navigated from the panel and did nothing on a Docs page. It links by URL, since the manager's version SPA-navigates through `api.selectStory`, which is manager-api and unreachable from the preview iframe. A link that names its own origin is left alone, where it was rebased onto the Storybook root and `[MDN](https://developer.mozilla.org/...)` took the whole tab to `<storybook-origin>/https://developer.mozilla.org/...`; absolute targets carry `rel="noopener noreferrer"`, so a private Storybook host stays out of the Referer sent to a cited site.

  The Docs container passes the theme it is given through to Storybook's own container instead of dropping it and reverting every Docs page to light.

- The panel's nothing-to-show states use Storybook's own `EmptyTabContent`, so an empty Oversight tab reads like every other empty tab in the addons panel. The Docs block keeps its inline message, which suits the page it sits under, and `loading` is one of those states rather than a layout that shifts mid-load.

  A component with no description reads _None_, in italics, and stops; it named the file the component sits in, which the finding below already answers with what to do. A clean component reads `no findings` 👏 and stops too. A manifest whose prop payload the rules cannot read says "The prop rules did not run", rather than describing our own state in our own word for the data.

- A `docs-link-dangling` finding sets each manifest id it names as code, struck through in the same negative as the dead links in the description, with a mark carrying the reason for anyone who cannot see the strikethrough. The mark is spaced from the id, which read aloud ran together as one word, and an id that is a prefix of another id in the same finding is no longer truncated.
...
- Both surfaces read the manifest through one module, which owns the fetch, the one-per-page cache, and the rule that a failed fetch is never cached so a later mount retries. A manifest that is served but will not parse reports that, rather than the hint to enable the manifest feature: the server answered, so the feature is on, and the cause reaches the console instead of nowhere.
