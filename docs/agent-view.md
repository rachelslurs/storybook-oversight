...
`description` is the entry's, and only the entry's, on both sides. An empty string renders as no description at all, and neither the server nor `oversight-core` falls back to the payload's. Storybook fills the entry's from the story-meta JSDoc when a story file carries one and from the payload description otherwise, keeping the prose and moving every JSDoc tag into `jsDocTags`. A component documented only by tags therefore leaves an empty string on the entry and the raw tag text on the payload. `@describe` and `@desc` are the exception that still reaches the entry: their values are read ahead of the extracted prose, so tag text can legitimately be the description, and the server renders it.
