...
In the panel and the Docs block the findings read as a table: severity, rule, what happened, and the fix. Both tables in a report share one treatment, and each scrolls inside its own box rather than spilling out of the section on a narrow panel.
